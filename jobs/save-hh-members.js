...
upsertMany(
    ...
    state => {
        ...
        const azureHHmembers = dataArray.map((x, i) => {
          const [ start, simpleHHcode ] = String.split(x.pax_identifier_information.HHID_output, 'HH')
          console.log(simpleHHcode);
          
          const hhMemberId = `${simpleHHcode}-HH${i}`
...
/*(state.data.form.hh_members_registration).forEach( function(element){
  console.log(element.case.update.HHID_output);
  console.log(element.pax_identifier_information.PAX_info.first_name);
